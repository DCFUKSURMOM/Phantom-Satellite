...
class TestTimer(unittest.TestCase):
    def test_timer(self):
        ...
        self.assertEqual(timer.elapsed(), '00:00:03')
...
class TestInterpolate(unittest.TestCase):
    def test_interpolate_talos_is_always_defines(self):
        self.assertEqual(utils.interpolate('${talos}'), utils.here)

    def test_interpolate_custom_placeholders(self):
        self.assertEqual(utils.interpolate('${talos} ${foo} abc', foo='bar', unused=1),
                          utils.here + ' bar abc')


class TestParsePref(unittest.TestCase):
    def test_parse_string(self):
        self.assertEqual(utils.parse_pref('abc'), 'abc')

    def test_parse_int(self):
        self.assertEqual(utils.parse_pref('12'), 12)

    def test_parse_bool(self):
        self.assertEqual(utils.parse_pref('true'), True)
        self.assertEqual(utils.parse_pref('false'), False)
