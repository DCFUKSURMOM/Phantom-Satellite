...
def get_previous_version(version, partial_versions):
    ...
    We use Version for ESR because it can sort X.0bN lower than X.0
    ...
    """
    # Version truncates trailing zero in versions with more than 1
    # dot (unless it ends with 'esr'). Compose a structure that will be
    # sorted by StrictVersion and return untouched version.
    composed = sorted([(v, Version(v)) for v in partial_versions if
